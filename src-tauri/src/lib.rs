...
async fn connect(state: &AppState, database: Option<&str>) -> Result<Client> {
...
#[tauri::command]
async fn connect_server(state: tauri::State<'_, AppState>) -> Result<ConnectionInfo> {
...
#[tauri::command]
async fn list_schemas(database: String, state: tauri::State<'_, AppState>) -> Result<Vec<SchemaInfo>> {
    let client = connect(&state, Some(&database)).await?;
...
#[tauri::command]
async fn get_table_metadata(request: TableRequest, state: tauri::State<'_, AppState>) -> Result<TableMetadata> {
    let client = connect(&state, Some(&request.database)).await?;
...
#[tauri::command]
async fn get_rows(request: PageRequest, state: tauri::State<'_, AppState>) -> Result<RowPage> {
    ...
    for row in query_rows {
        ...
        let row_ref = if pk.is_empty() { None } else {
            ...
            Some(state.signer.sign(&RowRef { database: request.database.clone(), schema: request.schema.clone(), table: request.table.clone(), key })?)
        };
...
#[tauri::command]
async fn insert_row(request: InsertRequest, state: tauri::State<'_, AppState>) -> Result<()> {
...
#[tauri::command]
async fn update_row(request: UpdateRequest, state: tauri::State<'_, AppState>) -> Result<()> {
    if request.values.is_empty() { return Err("No values changed".into()); }
    let reference = state.signer.verify(&request.row_ref)?;
    let client = connect(&state, Some(&reference.database)).await?;
...
#[tauri::command]
async fn delete_row(request: DeleteRequest, state: tauri::State<'_, AppState>) -> Result<()> {
...
mod tests {
    ...
    fn row_refs_round_trip_and_reject_tampering() {
        ...
        let reference = RowRef { database: "db".into(), schema: "public".into(), table: "people".into(), key: BTreeMap::from([("id".into(), Value::from(42))]) };
